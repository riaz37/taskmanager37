...
export const useEnhancedThemeStore = create<EnhancedThemeState>()((set, get) => ({
  mounted: false,
  ...
  toggleThemeWithTransition: async (event?: React.MouseEvent) => {
    ...
    try {
      ...
      console.warn('Theme transition failed, falling back to direct theme change:', error);
    } finally {
...
export function useThemeTransitions() {
  ...
  const setThemeWithTransition = async (newTheme: string, event?: React.MouseEvent) => {
    if (isTransitioning) return;
